...
def extentd_video(video, faces, cropped):
    video_reverted = video[::-1]    #revert video
    faces_reverted = faces[::-1]    #revert faces
    cropped_reverted = cropped[::-1]    #revert cropped

    combined_video = video + video_reverted + video + video_reverted + video #merge two list
    combined_faces = faces + faces_reverted + faces + faces_reverted + faces #merge two list
    combined_cropped = cropped + cropped_reverted + cropped + cropped_reverted + cropped #merge two list


    return combined_video, combined_faces, combined_cropped
...
#datagen
def datagen(frames, mels, detect):
    ...
    if using_video_format:
        ...
        for i in range(0, len(frames)):
            ...
            try:
                ...
                #result = detect.detectMultiScale(frames[i],1.3,5)
                result = detect.detect(frames[i])[1][:4]
        ...
        try:
            #result, _, _, _ = yolo.detect(image)
            result = detect.detectMultiScale(frames[0],1.3,5)
    ...
    ################################################################    NEW PART TO SAVE TIME (EXTEND VIDEO) ##################################################
    frames, face_det_results, cropped = extentd_video(frames, face_det_results, cropped)
...
def start_generating(model, detect, full_frames, path_wav, face_enhancer, iteration):
    #outputs = []
    fps = 30
    ...
    #print("start datagen")
    gen = datagen(full_frames.copy(), mel_chunks, detect) ##returns the generator for lists: img_batch(faces), mel_batch(audio), frame_batch(original frames), coords_batch(coords of faces)
    #print("end datagen")
    ...
    for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen, total=int(np.ceil(float(len(mel_chunks))/batch_size)))):
        ...
        for p, f, c in zip(pred, frames, coords):
            ...
            try:
                ...
                print("Error on upscaling")
                #so we use the same frame without upscaling
    ...
    print("END!!!")
    #outputs = make_frames_HD(outputs)
    return result_path
