...
def check_faces(image, detect):
  ...
  try:
    ...
    #faces = detect.detectMultiScale(image,1.3,5)
...
def check_all_frames_statue(video, detect, model_back, model, device):
  results_HS = [0,0]
...
def check_all_frames_human(video, detect):
  #t = transforms.ToPILImage()
...
def check_single_frame_statue(frame, detect, model_back, model, device):
  indices_to_labels = ['arringatore', 'atena', 'atena_armata', 'demostene', 'dioniso', 'era_barberini', 'ercole', 'kouros_da_tenea', 'minerva_tritonia', 'poseidone', 'zeus', 'altro']
  #t = transforms.ToPILImage()

  #face detection
  if not check_faces(frame, detect):
    print("Error face not detected")
    return -1
...
def check_single_frame_human(frame, detect):
  #t = transforms.ToPILImage()
